...
#include "VecBosSelection/Selection/interface/ZpatFilterPf.h"
...
bool hltispresent3=1; //Necessary to correctly match the HLT
bool Debug3=false;    //Activate with true if you wonna have verbosity for Debug
  using namespace edm;
...
bool
ZpatFilterPf::filter (edm::Event & iEvent, edm::EventSetup const & iSetup)
{
  
  if (Debug3) cout<<"------- NEW Event -----"<<endl;
				...
				if(bit) {
				  ...
				  //If path is accepted, then together with its prescale it is stored in a map.
						...
						if(prescaleset!=-1) {
							...
							if (Debug3) cout<<"prescale.first "<<prescalepair.first<<" prescalepair.second "<<prescalepair.second<<endl;
						...
						if (Debug3) cout<<"prescale "<<prescale<<" minimal Prescale "<<minimalPrescale<<" for trigger "<<triggerNames.triggerName(itrig)<<endl;
  ...
  //totCont++;
  // if (electronCollection->size()!= pfElecCollection->size()){
//      pfPatDiff++; 
//      cout << "PF PAT size = "<<electronCollection->size()<<endl;
//      cout << "PF collection size = "<<pfElecCollection->size()<<endl;
//      if (electronCollection->size() < pfElecCollection->size()){pfPatDiffMinus++;}
//      else {pfPatDiffPlus++;}}

//   Handle < GsfElectronCollection > gsfElecCollection;
//   iEvent.getByLabel ("gsfElectrons", gsfElecCollection);
//   if (pfElecCollection->size()!= gsfElecCollection->size()){ 
//      cout << "GSF colelction size = "<<gsfElecCollection->size()<<endl;
//      cout << "PF collection size = "<<pfElecCollection->size()<<endl;
//      pfGsfDiff++;
//      if (gsfElecCollection->size() < pfElecCollection->size()){pfGsfDiffMinus++;}
//      else {pfGsfDiffPlus++;}
//   }
  ...
  for (pat::ElectronCollection::const_iterator recoElectron = electronCollection->begin (); recoElectron != electronCollection->end (); recoElectron++) {
    ...
    if ( SelectionUtils::DoWP80pf(recoElectron,iEvent) && SelectionUtils::DoHLTMatch(recoElectron,iEvent) && recoElectron->pt()>10.0){
      ...
      if (Debug3) cout<<" MMMM ele trigger size "<<recoElectron->triggerObjectMatches().size()<<endl;
	...
	if (highestptele->pt()<recoElectron->pt()){
      ...
      if (Debug3) cout<<"Tag IS NOT a WP80 electron...Exit"<<endl;
    }
  }
...
bool
ZpatFilterPf::beginRun(edm::Run &iRun, edm::EventSetup const& iSetup)
{
	...
	if (Debug3) cout<<"useAllTriggers? "<<useAllTriggers_<<endl;
...
DEFINE_FWK_MODULE (ZpatFilterPf);
