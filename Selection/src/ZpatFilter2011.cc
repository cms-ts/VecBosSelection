...
#include "VecBosSelection/Selection/interface/ZpatFilter2011.h"
...
bool hltispresent11=1; //Necessary to correctly match the HLT
bool Debug11=false;    //Activate with true if you wonna have verbosity for Debug
...
bool
ZpatFilter2011::filter (edm::Event & iEvent, edm::EventSetup const & iSetup)
{
				...
				if(bit) {
				  ...
				  //If path is accepted, then together with its prescale it is stored in a map.
						...
						if(prescaleset!=-1) {
							...
							if (Debug11) cout<<"prescale.first "<<prescalepair.first<<" prescalepair.second "<<prescalepair.second<<endl;
						...
						if (Debug11) cout<<"prescale "<<prescale<<" minimal Prescale "<<minimalPrescale<<" for trigger "<<triggerNames.triggerName(itrig)<<endl;
...
//******************************* SELECTIONS ***************************************************8
    ...
    if ( (!doID_ || ( (!useNewID_ && ((doWP90_ || SelectionUtils::DoWP80Pf(recoElectron,iEvent)) && 
	 ...
	 && SelectionUtils::DoHLTMatch(recoElectron,iEvent) && recoElectron->pt()>secondEleEnThrhold ){
      ...
      if (Debug11) cout<<"Tag is a WP80 electron..."<<endl;
      //Sort in Pt
      if (Debug11) cout<<"Electron pt value ->"<<recoElectron->pt()<<endl;
      if (Debug11) cout<<" MMMM ele trigger size "<<recoElectron->triggerObjectMatches().size()<<endl;
	...
	if (highestptele->pt()<recoElectron->pt()){
      ...
      if (Debug11) cout<<"Tag IS NOT a WP80 electron...Exit"<<endl;
    }
  }
...
bool
ZpatFilter2011::beginRun(edm::Run &iRun, edm::EventSetup const& iSetup)
{
	...
	//debug dav
	if(hlNames.size()==0) { 
		hltispresent11=false;
	}

	if (Debug11) cout<<"useAllTriggers? "<<useAllTriggers_<<endl;
...
DEFINE_FWK_MODULE (ZpatFilter2011);
