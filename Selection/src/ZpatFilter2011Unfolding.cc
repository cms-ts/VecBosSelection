...
#include "VecBosSelection/Selection/interface/ZpatFilter2011Unfolding.h"
...
bool hltispresent111=1; //Necessary to correctly match the HLT
bool Debug111=false;    //Activate with true if you wonna have verbosity for Debug
...
bool
ZpatFilter2011Unfolding::filter (edm::Event & iEvent, edm::EventSetup const & iSetup)
{
  ...
  if (isUnfolding_){
	...
	if (Debug111) cout<<"lepton id->"<<itgen->pdgId()<<" eta"<<itgen->eta()<<" et->"<<itgen->et()<<" mass->"<<itgen->mass()<<endl;
      } 
    }
    ...
    if (leptonContainer.size()>=2) {
      ...
      zInvMass = l_pair.M();
      if (Debug111) cout<<" invMass->"<<zInvMass<<endl;  
	...
	if (Debug111) cout<<"This is a well generated Z Boson that you sohuld have reconstructed..."<<endl;
	valueToBeReturnedGen=true;
      }
      else{
	valueToBeReturnedGen=false;
	if (Debug111) cout<<"This is not a Z Boson inside the acceptance"<<endl;
      }
    }
    else{
      valueToBeReturnedGen=false;
...
if (Debug111)  cout<<"What gen level returns->"<<valueToBeReturnedGen<<endl;
  ...
  if (Debug111) cout<<"------- NEW Event -----"<<endl;
  eleSelStepByStep->SetBinContent(1,eleSelStepByStep->GetBinContent(1)+1); //Total Number of events (1)
	...
	if (HLTResults.isValid() && doTheHLTAnalysis_) {
				...
				if(bit) {
				  ...
				  //If path is accepted, then together with its prescale it is stored in a map.
						...
						if(prescaleset!=-1) {
							...
							if (Debug111) cout<<"prescale.first "<<prescalepair.first<<" prescalepair.second "<<prescalepair.second<<endl;
						...
						if (Debug111) cout<<"prescale "<<prescale<<" minimal Prescale "<<minimalPrescale<<" for trigger "<<triggerNames.triggerName(itrig)<<endl;
		...
		if (!flag) 
		{
			if(!useAllTriggers_) return valueToBeReturnedGen;
...
//******************************* SELECTIONS ***************************************************8
    ...
    if ( (!doID_ || ( (!useNewID_ && ((doWP90_ || SelectionUtils::DoWP80Pf(recoElectron,iEvent)) && 
	 ...
	 && SelectionUtils::DoHLTMatch(recoElectron,iEvent) && recoElectron->pt()>secondEleEnThrhold ){
      ...
      if (Debug111) cout<<"Tag is a WP80 electron..."<<endl;
      //Sort in Pt
      if (Debug111) cout<<"Electron pt value ->"<<recoElectron->pt()<<endl;
      if (Debug111) cout<<" MMMM ele trigger size "<<recoElectron->triggerObjectMatches().size()<<endl;
	...
	if (highestptele->pt()<recoElectron->pt()){
      ...
      if (Debug111) cout<<"Tag IS NOT a WP80 electron...Exit"<<endl;
    }
  }
  ...
  if(i<2 || highestptele->pt()<firstEleEnThrhold) return valueToBeReturnedGen; //you NEED at least two electrons :)
  ...
  if(highestptele->charge() == secondptele->charge()) return valueToBeReturnedGen;
  ...
  if (Debug111)  cout<<"This is a well reco Z"<<endl;
  return true;
...
void
ZpatFilter2011Unfolding::beginJob (){
  cout<<endl;
  cout<<"##############################"<<endl;
...
bool
ZpatFilter2011Unfolding::beginRun(edm::Run &iRun, edm::EventSetup const& iSetup)
{

	hltispresent111=true;
	//HLT names
	std::vector<std::string>  hlNames;
	hlNames.clear();
	bool changed (true);
	...
	//debug dav
	if(hlNames.size()==0) { 
		hltispresent111=false;
	}

	if (Debug111) cout<<"useAllTriggers? "<<useAllTriggers_<<endl;
...
void
ZpatFilter2011Unfolding::endJob ()
{
...
DEFINE_FWK_MODULE (ZpatFilter2011Unfolding);
